...
void CEngineStats::GetSyncInfo(CAESyncInfo& info, CActiveAEStream *stream)
{
  ...
  if (stream->m_pClock)
  {
    if (stream->m_syncClock)
      info.state = CAESyncInfo::SNYC_ACTIVE;
    else
...
CSampleBuffer* CActiveAE::SyncStream(CActiveAEStream *stream)
{
  ...
  if (newerror && fabs(error) > 100 && !stream->m_syncClock)
  {
    stream->m_syncClock = true;
    stream->m_resampleBuffers->m_resampleRatio = 1.0;
    CLog::Log(LOGDEBUG,"ActiveAE - average error %f above threshold of %f", error, 100.0);
  }
  ...
  if (stream->m_syncClock)
  {
    ...
    else
    {
      ...
      {
        memmove(buf->pkt->data[i], buf->pkt->data[i]+bytesToSkip, bytesToSkip);
      }
