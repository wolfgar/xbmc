...
#include "DirectoryNodePictureTop100.h"
...
#include "pictures/PictureDbUrl.h"
...
/*
        ...
        case NODE_TYPE_ALBUM_RECENTLY_PLAYED_PICTURES:
            return new CDirectoryNodeAlbumRecentlyPlayedPicture(strName, pParent);
 ...
 */
        ...
        case NODE_TYPE_YEAR_PICTURE:
            return new CDirectoryNodeYearPicture(strName, pParent);
        default:
            break;
    }
    return NULL;
...
void CDirectoryNode::AddQueuingFolder(CFileItemList& items) const
{
    ...
    CPictureDbUrl musicUrl;
    if (!musicUrl.FromString(BuildPath()))
        return;
    
    // always hide "all" items
    if (g_advancedSettings.m_bPictureLibraryHideAllItems)
        return;
    ...
    switch (GetChildType())
    {
        ...
        case NODE_TYPE_YEAR_ALBUM:
            ...
            /*  case NODE_TYPE_ALBUM_RECENTLY_PLAYED_PICTURES:
             ...
             case NODE_TYPE_PICTURE_TOP100:
             case NODE_TYPE_PICTURE:
             pItem = new CFileItem(g_localizeStrings.Get(15104));  // "All Pictures"
             musicUrl.AppendPath("-1/");
             pItem->SetPath(musicUrl.ToString());
             break;*/
        default:
    ...
    if (pItem)
    {
        ...
        pItem->SetSpecialSort(g_advancedSettings.m_bPictureLibraryAllItemsOnBottom ? SortSpecialOnBottom : SortSpecialOnTop);
        pItem->SetCanQueue(false);
        pItem->SetLabelPreformated(true);
        if (g_advancedSettings.m_bPictureLibraryAllItemsOnBottom)
            items.Add(pItem);
        else
